    ...
    <div className="flex flex-col gap-3 py-10">
     ...
     <div className="flex flex-wrap gap-3">
      {project.techStack.map((tech: string) => (
       <span
        key={tech}
        className="font-sans text-[0.72rem] tracking-wide mt-2 uppercase border border-ink/20 text-ink/70 px-3 py-1 rounded-sm hover:border-rust hover:text-rust transition-colors duration-200"
       >
        {tech}
       </span>
      ))}
   ...
   <div className="flex flex-wrap items-center gap-4 pt-2">
    ...
    {hasRepo && (
     <Link
      ...
      className="font-sans text-[0.78rem] tracking-widest uppercase font-medium bg-rust text-cream px-7 py-4 rounded-sm hover:bg-rust-dark hover:text-cream transition-colors duration-300"
     >
      View on GitHub ↗
     </Link>
    )}
